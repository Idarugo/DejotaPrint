...
async function updateTrabajadore(req, res) {
  ...
  const { nombre, apellido, telefono, nuevaContraseña } = req.body;
  ...
  try {
    ...
    const updatedTrabajadoreData = {
      nombre: nombre || existingTrabajadore.nombre,
      apellido: apellido || existingTrabajadore.apellido,
      telefono: telefono || existingTrabajadore.telefono,
      password: nuevaContraseña
        ? await bcrypt.hash(nuevaContraseña, 10)
        : existingTrabajadore.password,
    };
...
async function getCurrentUser(req, res) {
  try {
    ...
    const decodedToken = jwt.verify(token, "tu_secreto_jwt");
...
async function getTrabajadorByRut(req, res) {
  try {
    const { rut } = req.params;
    const trabajador = await trabajadoreModel.getTrabajadoreByRut(
      req.pool,
