...
async function createTestimonio(pool, testimonioData) {
  const { nombre, imagen_url, mensaje, estado } = testimonioData;
  const sql =
    "INSERT INTO testimonios (nombre, imagen_url, mensaje, estado) VALUES (?, ?, ?, ?)";
  return pool.query(sql, [nombre, imagen_url, mensaje, estado]);
}
