...
async function updateTrabajadore(pool, trabajadoreId, trabajadoreData) {
  const {
    rut,
    nombre,
    apellido,
  ...
  const sql =
    "UPDATE trabajadores SET rut = ?, nombre = ?, apellido = ?, telefono = ?, correo = ?, posicion = ?, password = ?, estado = ? WHERE id = ?";
  try {
    const connection = await pool.getConnection();
    const [result] = await connection.query(sql, [
      rut,
